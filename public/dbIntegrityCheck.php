...
function ciniki_gallery_dbIntegrityCheck($ciniki) {
    ...
    $rc = ciniki_gallery_checkAccess($ciniki, $args['tnid'], 'ciniki.gallery.dbIntegrityCheck', 0);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $args['fix'] == 'yes' ) {
        ...
        foreach($objects as $o => $obj) {
            $rc = ciniki_core_objectRefFix($ciniki, $args['tnid'], 'ciniki.gallery.'.$o, 0x04);
            if( $rc['stat'] != 'ok' ) {
                return $rc;
            }
        }
        ...
        //
        $rc = ciniki_core_dbFixTableHistory($ciniki, 'ciniki.gallery', $args['tnid'],
